...
class FamilyPage extends StatefulWidget {
  ...
  const FamilyPage({super.key, required this.onBackToExplore});
...
class _FamilyPageState extends State<FamilyPage> {
  ...
  static const cardBg = Color(0xFFDFECFF); // pastel blue cards
  ...
  final List<_RentPost> posts = [
    _RentPost(
      title: "Family House",
      location: "Dum Dum Purba Sinthee, North 24 Parganas",
      price: "৳ 12,000 / month",
      distance: "7.24 km",
      image: "image/family_icon.jpg",
    ),
    _RentPost(
      title: "Family House",
      location: "Barasat Railway Station, North 24 Pgs",
      price: "৳ 15,000 / month",
      distance: "6.55 km",
  ...
  // ✅ Explore does nothing, Home goes to Explore page
  void _onBottomTap(int idx) {
    setState(() => bottomIndex = idx);

    // Explore tap -> do nothing
    if (idx == 0) return;

    // Home tap -> go back to ExplorePage
    if (idx == 1) {
          ...
          content: SingleChildScrollView(
            child: Column(
              children: [
                ...
                TextField(
                  controller: priceC,
                  decoration: const InputDecoration(labelText: "Price (e.g. ৳ 12000 / month)"),
                ),
                TextField(
                  controller: distC,
                  decoration: const InputDecoration(labelText: "Distance (e.g. 7.2 km)"),
                ),
              ],
            ),
          ),
    ...
    if (res == true) {
      setState(() {
        posts.insert(
          0,
          _RentPost(
            title: titleC.text.trim().isEmpty ? "Family House" : titleC.text.trim(),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
            ...
            Row(
              children: [
                ...
                Text(
                  "${posts.length} posts",
                  style: TextStyle(
                    color: oceanMuted.withOpacity(0.9),
                    fontWeight: FontWeight.w900,
                  ),
                ),
              ],
      ...
      // ✅ Bottom bar (Explore does nothing, Home goes Explore)
        ...
        child: SizedBox(
          ...
          child: Row(
            ...
            children: [
              ...
              const SizedBox(width: 46), // FAB gap
              _NavItem(
...
class _RentCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      onTap: () {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("${post.title} clicked")),
        );
      },
      child: Container(
        decoration: BoxDecoration(
