...
class BachelorPage extends StatefulWidget {
  ...
  const BachelorPage({super.key, required this.onBackToExplore});
...
class _BachelorPageState extends State<BachelorPage> {
  ...
  final List<_RentPost> posts = [
    _RentPost(
      title: "Bachelor Room (Single)",
      location: "Mirpur 10, Dhaka",
      price: "৳ 6,500 / month",
      distance: "2.80 km",
      image: "image/bachelor_icon.jpg",
    ),
    _RentPost(
      title: "Bachelor Flat (Shared)",
  ...
  void _onBottomTap(int idx) {
    ...
    if (idx == 0) return;
    if (idx == 1) {
      widget.onBackToExplore();
      return;
    }
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(idx == 2 ? "Message clicked" : "Profile clicked")),
    );
  }

  void _filterAction() {
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Filter clicked")));
  }
  ...
  Future<void> _addPostDialog() async {
    ...
    final res = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        title: const Text("Add Bachelor Rent Post"),
        content: SingleChildScrollView(
          child: Column(
            children: [
              ...
              TextField(controller: priceC, decoration: const InputDecoration(labelText: "Price (e.g. ৳ 6500 / month)")),
              TextField(controller: distC, decoration: const InputDecoration(labelText: "Distance (e.g. 2.8 km)")),
            ],
          ),
        ),
        actions: [
    ...
    if (res == true) {
      setState(() {
        posts.insert(
          0,
          _RentPost(
            title: titleC.text.trim().isEmpty ? "Bachelor Room" : titleC.text.trim(),
  ...
  void _deletePost(int i) {
    final removed = posts[i];
    setState(() => posts.removeAt(i));
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text("Deleted: ${removed.title}"),
        action: SnackBarAction(label: "Undo", onPressed: () => setState(() => posts.insert(i, removed))),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text("Bachelor Rentals", style: TextStyle(color: oceanDark, fontWeight: FontWeight.w900)),
        actions: [
          IconButton(onPressed: _filterAction, icon: const Icon(Icons.tune_rounded, color: ocean)),
          ...
          children: [
            Row(
              children: [
                Container(
                  ...
                  child: const Text("Tap + to add post", style: TextStyle(color: oceanMuted, fontWeight: FontWeight.w800, fontSize: 12.5)),
                ),
                const Spacer(),
                Text("${posts.length} posts", style: TextStyle(color: oceanMuted.withOpacity(0.9), fontWeight: FontWeight.w900)),
              ],
            ),
            ...
            Expanded(
              child: GridView.builder(
                ...
                itemBuilder: (_, i) => _RentCard(post: posts[i], onDelete: () => _deletePost(i)),
              ),
            ),
          ],
        ),
